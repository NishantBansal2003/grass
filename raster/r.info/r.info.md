...
Output raster array information in plain format:
...
Output the map data range in plain format:
...
Output the extended map data metadata in plain format:
...
And, the full output from *r.info* can be output in shell by passing the
**format=shell** option.
